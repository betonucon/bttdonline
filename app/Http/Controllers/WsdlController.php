...
class WsdlController extends Controller
{
    ...
    public function parsing_bank(){
        ...
        $rows = explode("\n", trim($contents));
        $file=File::copy(public_path('SAPtoWeb/ZFI094_Vendor_Bank_Detail.txt'), public_path('arsip/ZFI094_Vendor_Bank_Detail'.date('ymd').'.txt'));
    ...
    public function parsing_vendor(){
        ...
        $rows = explode("\n", trim($contents));
        $file=File::copy(public_path('SAPtoWeb/ZFI094_Vendor_General_Master.txt'), public_path('arsip/ZFI094_Vendor_General_Master'.date('ymd').'.txt'));
    ...
    public function parsing_pph(){
        ...
        $rows = explode("\n", trim($contents));
        $file=File::copy(public_path('SAPtoWeb/ZFI080_E-COUPON.txt'), public_path('arsip/ZFI080_E-COUPON'.date('ymd').'.txt'));
            foreach($rows as $x=>$row) {
                if($x==0){
    ...
    public function parsing_voucher(){
        ...
        $rows = explode("\n", trim($contents));
        $file=File::copy(public_path('SAPtoWeb/ZFI004N_APPVOU.txt'), public_path('arsip/ZFI004N_APPVOU'.date('ymd').'.txt'));
            foreach($rows as $x=>$row) {
                $results= explode('|', trim($row));
                if($x==0){
